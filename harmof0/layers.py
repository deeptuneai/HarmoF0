...
class MRDConv(nn.Module):
    def __init__(
        self, in_channels, out_channels, dilation_list=[0, 12, 19, 24, 28, 31, 34, 36]
    ):
        super().__init__()
...
class FRDConv(nn.Module):
    def __init__(
        self, in_channels, out_channels, kernel_size=[1, 3], dilation=[1, 1]
    ) -> None:
        super().__init__()
...
class WaveformToLogSpecgram(nn.Module):
    def __init__(
        ...
        self.waveform_to_specgram = torchaudio.transforms.Spectrogram(
