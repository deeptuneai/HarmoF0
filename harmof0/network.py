...
def dila_conv_block(
    ...
    if dilation_mode == "log_scale":
        ...
        dilation_list = a.round().astype(np.int32)
        ...
        dilation_list = np.array(
            [i * dil_kernel_size[1] for i in range(dil_kernel_size[1])]
        )
        causal_conv = FRDConv(
            out_channel, out_channel, dil_kernel_size, dilation=[1, dilation_rate]
        )
        return nn.Sequential(
            conv,
            nn.ReLU(),
