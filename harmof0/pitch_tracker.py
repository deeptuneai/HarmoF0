...
class PitchTracker:
    ...
    def pred(self, waveform, sr):
        ...
        waveform_pad = waveform[:, : self.frame_len // 2].flip(
            dims=[
                1,
            ]
        )
    ...
    def pred_file(self, audio_path, output_dir=None, save_activation=True):
        ...
        for i, wav_path in enumerate(wav_path_list):
            ...
            np.savetxt(
                pred_path, pred_table, header="time frequency activation", fmt="%.03f"
            )
            if save_activation:
                if self.post_processing == False:
                    activation_path = os.path.join(
                        result_dir, wav_name + ".activation.png"
                    )
                else:
                    activation_path = os.path.join(
                        result_dir, wav_name + ".activation.post.png"
                    )
